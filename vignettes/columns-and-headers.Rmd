...
`"bottom"`), `bold`, `background`, `color`, `font_size`, `repeat_on_page` (default
...
> then manual `COMPUTE` blocks to attach them. arframe does it in one
> `fr_cols()` call.
...
```{r spans-basic-preview, echo = FALSE}
spec
```
...
> `STYLE(SPANNING)`. arframe handles all positioning automatically from a
...
```{r col-split-preview, echo = FALSE}
spec
```
