...
as a header above each section.
...
For full control, use `fr_styles()` with `rows = "group_headers"`:
...
`suppress` only prints the value when it changes (like `NOREPEAT` in SAS).
...
> **Custom fonts for PDF:** Set `ARFRAME_FONT_DIR` to a directory of `.ttf`/`.otf`
