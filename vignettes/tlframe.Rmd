...
> **SAS parallel:** `TITLE1 "Table 14.1.5"; TITLE2 "Demographics...";`
...
The `format` template has two tokens: `{name}` (the column label) and `{n}`
(the count). The default format is `"{name}\n(N={n})"`.
...
For maximum flexibility, pass a function that receives the page group's data
and label:
...
automatically. This is the recommended approach for vital signs and lab tables.
...
```{r listing-demo}
listing_data <- adsl[1:20, c("USUBJID", "ARM", "AGE", "SEX")]
spec <- listing_data |>
  fr_listing() |>
  fr_rows(sort_by = c("ARM", "USUBJID"), repeat_cols = "ARM")
```
...
| Table number and title | `fr_titles("Table 14.1.5", "Demographics...")` |
...
spec <- tbl_ae_soc |>
  fr_table() |>
  fr_titles("Table 14.3.1.3", "TEAEs by SOC and Preferred Term") |>
  fr_page(continuation = "(continued)")
# Page 2+ will show: "Table 14.3.1.3 (continued)"
```
...
tbl_demog |>
  fr_table() |>
  fr_titles("Table 14.1.5", "Demographics") |>
  fr_hlines("header") |>
...
wide_spec <- tbl_vs[tbl_vs$timepoint == "Week 24" &
  ...
  fr_table() |>
  fr_titles("Table 14.3.5.1", "Systolic BP --- Column Split") |>
  fr_cols(
    param     = fr_col(visible = FALSE),
    timepoint = fr_col(visible = FALSE),
