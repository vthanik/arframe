---
title: "Best Practices & Pharmaverse Integration"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Best Practices & Pharmaverse Integration}
...
arframe discovers the file by walking up from the working directory (like
`.Rprofile`). You never need to specify the path.
...
| Session overrides | `fr_theme()` | Quick experiments, temporary changes |
| Per-table overrides | `fr_page()`, `fr_header()`, etc. | Only what's unique to one table |
...
session defaults with `fr_theme()`:
...
produces Analysis Results Data (ARD). `fr_wide_ard()` converts ARD
output to a wide data frame ready for `fr_table()`.

### Demographics (continuous + categorical)

```{r cards-demog, eval = FALSE}
library(cards)
...
grouping with `fr_rows()`.

### Per-variable format overrides

```{r cards-per-var, eval = FALSE}
fr_wide_ard(
  demog_ard,
  statistic = list(
    AGE = c(
...
Register custom formats when your data uses stat patterns not covered
by the 18 built-in types:

```{r custom-stat, eval = FALSE}
