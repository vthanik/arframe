...
|------------|-------|----------|
| `fr_row_style()` | Entire row(s) | `rows`, `bold`, `background`, `color`, `align`, `valign`, `height`, ... |
...
| `fr_col_style()` | Entire column(s) | `cols`, `bold`, `background`, `color`, `align`, `valign`, ... |
| `fr_style()` | Row-column intersection | `region`, `rows`, `cols`, `bold`, `background`, `color`, `align`, `valign`, `colspan`, ... |
| `fr_style_if()` | Data-driven (conditional) | `condition`, `cols`, `apply_to`, `bold`, `background`, `color`, `align`, ... |
...
When `fr_rows()` creates group headers (via `group_by` with `label` or
`leaf`), style them with `group_style` on `fr_rows()` or
`rows = "group_headers"` in `fr_styles()`:

```{r group-header-quick}
...
```{r group-header-level, eval = FALSE}
spec |>
  fr_styles(
...
`background`, `font_size`, `align`. Consistent across RTF, HTML, and PDF.
...
```{r rows-matches-preview, echo = FALSE}
spec
```
...
```{r header-region-preview, echo = FALSE}
spec
```
...
> **SAS:** `COMPUTE category; IF category = "Randomized" THEN CALL DEFINE(_ROW_, "STYLE", "STYLE=[FONT_WEIGHT=BOLD BACKGROUND=GRAY]"); ENDCOMP;`
