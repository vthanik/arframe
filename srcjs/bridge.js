...
// The ONE variable-picker render, shared by every arframe variable/param
// selectize (Roles, Filters, Populations subject-id, Treatment, Continuous
// stats, Decimals). Defined here ONCE and exposed as globals; R passes only
// `render = I("{ option: window.arframePickerOption, item:
// window.arframePickerItem }")` -- no render markup lives in the R modules.
// Each selectize choice's LABEL is packed `name\x1ftype\x1flabel` (\x1f = unit
// separator, absent from any name/type/label); the render splits it back into
// a type-chip avatar + NAME + muted CDISC label on one line.
//
// Chip per column kind. Two vocabularies flow in and both are honoured: the
// arpillar `data_items` taxonomy (measure | category | date -- it collapses
// time/datetime into date) and datasetviewer's finer Arrow kinds (number |
// string | bool | date | datetime | time). Glyphs mirror datasetviewer's
// typeIcon: # for number, a monochrome calendar for date/datetime, a clock
// for time-of-day, A for char -- all currentColor, flat, no emoji. `param`
// is arframe's own BDS-parameter chip. Unknown -> category.
var AR_CAL =
  '<svg viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">' +
  '<rect x="2" y="3" width="12" height="11" rx="1.5" fill="none" stroke="currentColor" stroke-width="1.3"/>' +
  '<path d="M2 6 H14" stroke="currentColor" stroke-width="1.3"/>' +
  '<path d="M5 1.5 V4 M11 1.5 V4" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/></svg>';
var AR_CLOCK =
  '<svg viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">' +
...
window.arframePickerOption = function (item, escape) {
  var p = arPickerParts(item);
  var t = AR_PICKER_TYPE[p[1]] || AR_PICKER_TYPE.category;
  var lab = p[2] || "";
  return (
    '<div class="ar-picker-option">' +
    '<span class="' +
    t.cls +
    '">' +
    t.gl +
    "</span>" +
    '<div class="ar-picker-option-text">' +
    '<span class="ar-picker-option-name">' +
    escape(p[0]) +
    "</span>" +
    (lab
      ? '<span class="ar-picker-option-lab">' + escape(lab) + "</span>"
      : "") +
    "</div></div>"
  );
};
window.arframePickerItem = function (item, escape) {
  return (
    '<div class="ar-picker-item">' + escape(arPickerParts(item)[0]) + "</div>"
  );
};
